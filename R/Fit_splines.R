...
Fit_splines_single = function(data, Time_col, OD_col, degrees_freedom){
  # Extract data
  x_val = dplyr::pull(data, Time_col)
  y_val = dplyr::pull(data, OD_col)
  # Fit splines
  spln_1 =  stats::smooth.spline(x_val, y_val, df = degrees_freedom[1])
  spln_2 =  stats::smooth.spline(x_val, y_val, df = degrees_freedom[2])
  #Extract fit and residuals and output single df
  out_tmp = broom::augment(spln_1, data) %>% 
...
Fit_splines = function(data, Time_col = "Time", OD_col = "OD", degrees_freedom = c(4,10), group_var = "RRPPRCC") {
  splines_df = data %>% 
    dplyr::group_split(!!rlang::sym(group_var)) %>% 
    map_df(function(df) Fit_splines_single(df, Time_col = Time_col, OD_col = OD_col,  degrees_freedom = degrees_freedom))
