...
and data), `conflicted` (to solve possible conflicts in function assignations),
and `ggplot2` to generate plots. Finally, we will also use the `readxl` package, 
which you should also already have.
```{r}
# R functions
library(neckaR)
```
...
adjusted_df %>% 
  dplyr::filter(Control== TRUE, cutoff_time <= 12) %>% 
  dplyr::count(RRPP) %>% 
  dplyr::select(-n)
```
...
Max_OD_df = filtered_marked_df %>% 
	...
	dplyr::ungroup() %>% 
	dplyr::select(RRPPRCC, Drug, Drug_name, Replicate, Max_OD = OD, Max_ODc01 = ODc01, cutoff_time, lag_time)  
...
# Combined df
final_df = dplyr::left_join(Max_OD_df, selected_AUC_df, by = "RRPPRCC")
